...
//                          g_subs.at(j).at(0))) {  // index gss と gall
//                          の不一致
//           is_Gd.at(i) = false;
//           break;
//         }
...
// vector<int> make_count_DP(const vector<vector<uint8_t>> &data, vector<int>
// &Gs,
//                           vector<int> &n_states, int &parallel) {
//   // Gs全ノードの頻度表を計算
//   int x_len = 1;
...
// vector<vector<int>> state_count_DP(vector<int> &children, vector<int>
// &parents,
//                                    vector<int> &n_states, vector<int>
//                                    &count_DP, vector<int> &Gs, int &parallel)
//                                    {
...
//           y_val =
//               n_states.at(parents.at(j)) * y_val +
//               Gs_val.at(parents_Gs.at(j));
//         }
...
//         x_val =
//             n_states.at(children.at(j)) * x_val +
//             Gs_val.at(children_Gs.at(j));
//       }
//       if (!parents.empty()) {
//         y_val = Gs_val.at(parents_Gs.at(0));
...
// float natori_independent_score(const vector<vector<uint8_t>> &data, int
// &node_x,
//                                int &node_y, vector<int> &parents,
//                                vector<int> &n_states, float &ESS, int
//                                &parallel, bool &count_DP_flag, vector<int>
//                                &count_DP, vector<int> &Gs) {
...
//     if (count_DP_flag) {
//       count =
//           state_count_DP(node_x_vec, parents, n_states, count_DP, Gs,
//           parallel);
//     } else {
...
//     if (count_DP_flag) {
//       count2 =
//           state_count_DP(node_y_vec, parents, n_states, count_DP, Gs,
//           parallel);
//     } else {
...
//         if (isnan(score)) {
//           cout << ESS / (r * q) + count.at(k).at(j) << "score is nan" <<
//           endl;
//         }
//       }
...
//         if (isnan(score)) {
//           cout << ESS / (r * q) + count2.at(k).at(j) << "score is nan" <<
//           endl;
//         }
//       }
...
// float natori_dependent_score(const vector<vector<uint8_t>> &data, int
// &node_x,
//                              int &node_y, vector<int> &parents,
//                              vector<int> &n_states, float &ESS, int
//                              &parallel, bool &count_DP_flag, vector<int>
...
//     if (count_DP_flag) {
//       count =
//           state_count_DP(children, parents, n_states, count_DP, Gs,
//           parallel);
//     } else {
...
//              vector<int> &Z, vector<int> &n_states, float &ESS, int
//              &parallel, bool &count_DP_flag, vector<int> &count_DP,
//              vector<int> &Gs, int &n_citest, int &n_citest_DP,
//              vector<vector<vector<bool>>> &sep, vector<vector<bool>> &sep2) {
...
//   independent_score +=
//       natori_independent_score(data, node_x, node_y, Z, n_states, ESS,
//       parallel,
//                                count_DP_flag, count_DP, Gs);
//   dependent_score +=
//       natori_dependent_score(data, node_x, node_y, Z, n_states, ESS,
//       parallel,
//                              count_DP_flag, count_DP, Gs);
...
//      Artificial Inteligence (UAl-95),p.403-410 (195) in this stage (stage A2
//      in Yehezkel and Lerner(2009)), only rule 1 is applied because only X ->
//      Y - Z shape is created in stage A1 (X-Z removed).
//   */
...
//                     vector<vector<bool>> &deletededges,
//                     const vector<vector<uint8_t>> &data, vector<int>
//                     &n_states, float &ESS, int &parallel, bool
//                     &count_DP_flag, vector<int> &count_DP,
//                     vector<vector<vector<bool>>> &sep, vector<vector<bool>>
//                     &sep2) {
//   /*
//       orient edges in a PDAG_nishikori to a maximally oriented graph.
//       orient rules are based on rule 1~3 from Meek,C.:Causal Inference and
...
// void recursive_search(const vector<vector<uint8_t>> &data, PDAG_nishikori
// &Gall,
...
//                       count_DP_flag, count_DP, Gs, n_citest, n_citest_DP,
//                       sep, sep2)) {
//             Gall.remove_edge(node_x, node_y);
...
//                      do_orientation_A2, sep, sep2, n_citest, n_citest_DP,
//                      n_DP);
//   }
...
//                    threshold_DP, search_neighbor, do_orientation_A2, sep,
//                    sep2, n_citest, n_citest_DP, n_DP);
//   return;
// }
...
//   // translate imput data to c++ vector(this is not optimal but I don't know
//   how
//   // to use pybind11::array_t)
//   py::buffer_info buf_data = data.request(), buf_states = n_states.request();
//   const uint8_t *__restrict__ prt_data = static_cast<uint8_t
//   *>(buf_data.ptr); const int *__restrict__ prt_states = static_cast<int
//   *>(buf_states.ptr); int n_data = buf_data.shape[0],
...
//   int sep_size = n_node;
//   vector<vector<vector<bool>>> sep(
//       sep_size, vector<vector<bool>>(sep_size, vector<bool>(sep_size,
//       false)));
...
//   // translate Gall to py::array_t (this is not optimal but I don't know how
//   to
...
//     }
//   }
//   cout << "citest_count: " << n_citest << ", citest_DP_count: " <<
//   n_citest_DP
