...
void orientation_B2(PDAG &Gall, vector<int> &Gs,
  ...
  */
  ...
  while (flag) {
    ...
    //   for (auto &Y : Gs) {
    //     for (auto &Z : Gs) {
    //       if (!Gall.has_edge(X, Z) && !Gall.has_edge(Z, X) && Z != X && Z != Y && X != Y && Gall.has_edge(X, Y) && !Gall.has_edge(Y, X) && Gall.has_edge(Y, Z) && Gall.has_edge(Z, Y)) {
    //         Gall.remove_edge(Z, Y);
    //         // cout<< "R1:" <<Y<<"->"<<Z<<"|"<<X<< endl;
    //         flag = true;
    //       }
    ...
    for (auto &X : Gs) {
      for (auto &Y : Gall.undirected_neighbors(X)) {
        for (auto &Z : Gall.undirected_neighbors(Y)) {
          if (Z != X && !Gall.has_edge(Z, X) && !Gall.has_edge(X, Z)) {  // X-Y-Z
