...
// float natori_independent_score(const vector<vector<int>> &data, int &node_x, int &node_y, vector<int> &parents, vector<int> &n_states) {
...
//         score += lgamma(alpha + count.at(k).at(0)) - lgamma(r * alpha + n_i);
//     }
//     score += lgamma(r * alpha) - lgamma(alpha);
...
//     }
//     for (int k = 0; k < r; k++) { //for each state of node_x
//         score += lgamma(alpha + count2.at(k).at(0)) - lgamma(r * alpha + n_i);
//     }
//     score += lgamma(r * alpha) - lgamma(alpha);
//   }
//   else {
...
//       for (int k = 0; k < r; k++) { //for each state of node_x
//         score += lgamma(alpha + count.at(k).at(j)) - lgamma(r * alpha + n_ij.at(j));
//         if (isnan(score)) {
...
//     for (int j = 0; j < q; j++) { //for each state of parents
//       for (int k = 0; k < r; k++) { //for each state of node_x
//         score += lgamma(alpha + count2.at(k).at(j)) - lgamma(r * alpha + n_ij.at(j));
//         if (isnan(score)) {
...
// float natori_dependent_score(const vector<vector<int>> &data, int &node_x, int &node_y, vector<int> &parents, vector<int> &n_states) {
//   //return log of the BDeu score of X, Y | Z
//   double score = 0.0;
//   double alpha = 0.5; // hyperparameter
...
//     }
//     for (int k = 0; k < r; k++) { //for each state of node_x
//         score += lgamma(alpha + count.at(k).at(0)) - lgamma(n_i + r * alpha);
//     }
//     score += lgamma(r * alpha) - lgamma(alpha);
//   }
//   else {
...
bool ci_test(const vector<vector<int>> &data, int &node_x, int &node_y, vector<int> &Z, vector<int> &n_states) {
  //CI test for X _|_ Y | Z
  float independent_score = 0.0;
  float dependent_score = 0.0;
  independent_score += natori_independent_score(data, node_x, node_y, Z, n_states);
  dependent_score += natori_dependent_score(data, node_x, node_y, Z, n_states);
...
void orientation_B2(PDAG &Gall, vector<int> &Gs, vector<vector<bool>> &deletededges, const vector<vector<int>> &data, vector<int> &n_states) {
  /*
      orient edges in a PDAG to a maximally oriented graph.
      orient rules are based on rule 1~3 from Meek,C.:Causal Inference and Causal Explanation with Background Knowledge,Proc.Confon Uncertainty in Artificial Inteligence (UAl-95),p.403-410 (195)
  */
    ...
    for (auto& Z : Gall.undirected_neighbors(X)) {
      for (auto& Y : Gall.undirected_neighbors(Z)) {
        if (X != Y && !Gall.has_edge(X, Y) && !Gall.has_edge(Y, X)) {
          ...
          if (!ci_test(data, X, Y, z, n_states)){
          //if (!ci_test_vstructure(data, X, Y, Z, ESS, n_states)){
...
PDAG recursive_search(const vector<vector<int>> &data, PDAG &Gall, vector<int> Gs, vector<int> Gex, int N, vector<int> &n_states) {
          ...
          if (Z.size() >= N) {
            do {
                  ...
                  if (!deletededges.at(node_x).at(node_y)){
                    if (ci_test(data, node_x, node_y, selected_z, n_states)) {
  ...
  for (auto& node_y : Gs) {
    for (auto& node_x : Gall.neighbors(node_y)) {
      if (!deletededges.at(node_x).at(node_y)){
        if (N == 0) {
          vector<int> S;
            if (ci_test(data, node_x, node_y, S, n_states)) {
            Gall.remove_edge(node_x, node_y);
  ...
  orientation_B2(Gall, Gs, deletededges, data, n_states);
  ...
  for (auto& Gex_i : g_subs) {
    ...
    Gall = recursive_search(data, Gall, Gex_i, Gex, N + 1, n_states);
  }
  //stage D:  Descendantsub-structure decomposition
  return recursive_search(data, Gall, Gd, Gexd, N + 1, n_states);
}
...
py::array_t<bool> RAI(py::array_t<int> data, py::array_t<int> n_states) {
  ...
  Gend = recursive_search(data_vec, Gall, Gs, Gex, 0, n_states_vec);
