...
void order_grouping(PDAG &Gall, vector<int> &Gs, vector<int> &Gd,
...
// bool ci_test_for_vstructure_detect_by_beyesfactor(const vector<vector<int>>
...
//   count_independent = state_count(data, children_independent,
//   parents_independent, n_states); int r = count_independent.size(); //number
//   of states of node_x int n_i = 0; for (int k = 0; k < r; k++) {
//     n_i += count_independent.at(k).at(0);
//   }
//   for (int k = 0; k < r; k++) { //for each state of node_x
//       independent_score += lgamma(alpha + count_independent.at(k).at(0)) -
//       lgamma(alpha);
//   }
...
//   dependent_score += localBDeuscore(data, node_z, parents_dependent_z,
//   n_states); dependent_score += localBDeuscore(data, node_x,
//   parents_dependent_x, n_states);
//   ////cout<< "xlocal"<<localBDeuscore(data, node_x, parents_dependent_x,
//   n_states)<<endl; dependent_score += localBDeuscore(data, node_y,
//   parents_dependent_y, n_states);
//   ////cout<< "ylocal"<<localBDeuscore(data, node_y, parents_dependent_y,
//   n_states)<<endl;
//   // float ess = -2.0;
//   // dependent_score += natori_dependent_score(data, node_x, node_y, zz,
//   n_states, ess); if(independent_score > dependent_score){
//     ////cout<< "CI independent:" <<node_x<<" _|_"<<node_y<<" |
//     "<<independent_score<<">"<<dependent_score<< endl; return false;
//   }else{
//     ////cout<< "CI dependent:" <<node_x<<" _|_"<<node_y<<" |
//     "<<independent_score<<"<"<<dependent_score<< endl; return true;
//   }
// }
...
//     for (int j = 0; j < r_y; j++) {
//       double temp = 0;
//       temp = p_xy.at(i * r_y + j) * log(p_xy.at(i * r_y + j) / (p_x.at(i) *
//       p_y.at(j))); cmi = cmi + temp;
//     }
//   }
...
void orientation_A2(PDAG &Gall, vector<int> &Gs,
