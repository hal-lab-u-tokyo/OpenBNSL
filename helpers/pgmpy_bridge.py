...
def to_openbnsl(pgmpy_pdag: PgmpyPDAG, col_str2idx: dict[str, int]) -> OpenBNSLPDAG:
    """
    ...
    if len(col_str2idx) != n:
        raise ValueError(
            f"Number of variable labels ({len(col_str2idx)}) does not match the number of nodes ({n})."
        )
